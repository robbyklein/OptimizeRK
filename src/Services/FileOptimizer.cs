...
public class FileOptimizer(Settings settings) {
	...
	public async Task OptimizeFilesAsync(IEnumerable<FileItem> files) {
		...
		var tasks = Enumerable.Range(0, _settings.MaxParallelism).Select(_ => Task.Run(async () => {
	...
	private async Task RunTool(FileItem file, string toolName) {
		...
		string args = toolName switch {
			"cjpeg-static.exe" =>
				$"-quality {_settings.JpgQuality} -outfile \"{tempOutput}\" \"{file.Path}\"",

			"pngquant.exe" =>
				$"--quality={Math.Max(0, _settings.PngQuality - 10)}-{_settings.PngQuality} --force -o \"{tempOutput}\" \"{file.Path}\"",

			"gifsicle.exe" =>
				$"--optimize=3 --lossy={_settings.GifQuality * 2} \"{file.Path}\" -o \"{tempOutput}\"",

			"ffmpeg.exe" =>
				$"-hide_banner -loglevel info -i \"{file.Path}\" -c:v libx264 -preset fast -crf {(int)(51 - _settings.Mp4Quality / 100.0 * 51)} -c:a copy -y \"{tempOutput}\"",
		...
		var finalOutput = _settings.ReplaceOriginalFile ? file.Path : tempOutput;

		if (_settings.ReplaceOriginalFile) {
